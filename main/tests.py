...
class main_Test_Cases(TestCase):
   ...
   def test_Category(self):
      ...
      self.category1 = Category.objects.create(title="testcategory", slug="testcategory", description="just a test") #create an category
      ...
      #test if the category appers in the main page of the forum
      ...
      self.assertTemplateUsed(response, 'main/forums.html') #checks that the template with the given name was used in rendering the response
   ...
   def test_Reply(self):
      ...
      self.reply1 = Reply.objects.create(user=author1 , content = "just a test") #create a replay
   ...
   def test_Comment(self):
      ...
      self.comment1 = Comment.objects.create(user=author1, content = "just a test") #create a comment
   ...
   def test_Post(self):
      ...
      self.post1 = Post.objects.create(title="test post", slug="testpost", user=author1,
      content= "just post for a test" , approved=True, tags="test") #create a post
      self.post1.categories.add(category1)#add a category to the post
      self.post1.save() #save the post with the new category
      self.assertEqual(Post.objects.all().count(), 1)#count posts in the db should be 1
      self.assertEqual(Post.objects.filter(title="test post").count(), 1)#count comments with the title "test post" should be 1
      ...
      self.assertEqual(response.status_code, 200)#check if the page exits or there is an error
      self.assertTemplateUsed(response, 'main/posts.html')#checks that the template with the given name was used in rendering the response
   ...
   #test if the post has its own page
      ...
      self.assertTemplateUsed(response, 'main/detail.html')#checks that the template with the given name was used in rendering the response
